...
<div>
  ...
  <modal name="walk-through" id="walk-through" :classes="['rounded']">
    ...
    <div class="mt-4" v-if="step == 4">
      ...
      <p>In this round, coins are passed on as soon as they are clicked. This is to
        simulate more of a Kanban or flow way of working</p>
      <p>There is no time limit on this round, but note the time it takes to deliver
    ...
    <div class="mt-4" v-if="step == 5">
      ...
      <p>This round is also kanban (coins passed on immediately),  but the timer stops
       ...
       <p>typically, despite having only one sixth of the time of round 1, most of the
         value - usually 80% to 85% - will be delivered. This will also typically be in
         50% or less of the time of round 2, so most of the value is delivered in
         much less time.</p>
    ...
    <div class="mt-4 conclusions" v-if="step == 7">
      ...
      <div class="row">
        ...
        <div class="col">
          <p>Show players the following graph and point out how it tails off - this is how we
            delivered 80% of the value in the final 10 second round, by delivering the
            highest value items first. </p>
          <p>If you draw the curve first, you can then add in the costs over time and show
            the point - X - at which the value we're delivering is less than the cost of
            delivering it. At this point (at the latest!), we should stop, or at least
            pivot onto something that delivers more value. </p>
        </div>
      </div>
    </div>
    <div class="mt-4 v conclusions" v-if="step == 8">
      <h4>Conclusions (2)</h4>
      <div class="row">
        <div class="col">
        ...
        <div class="col">
          <p>This graph shows that waiting until everything is done - Round One - is
            a huge risk (red hatching). If you take this approach and miss the deadline,
            you have spent all the money and delivered no value. The iterative approach
            reduces that risk to a small amount per iteration; whenever you stop
            development (after the first iteration) you have always delivered some
            value. And, of course, if you deliver the wrong thing, you've only wasted
            one iteration's worth of effort and can pivot onto the right thing
            very quickly...</p>
        </div>
      </div>
    </div>
    <div class="mt-4 v conclusions" v-if="step == 9">
      ...
      <div class="row">
        ...
        <div class="col">
          <p>This is a useful point to make in highly regulated industries, e.g.
            banking – an argument against delivering highest value first is often
            “it all has to be delivered anyway, so why bother?”. If you can deliver
            90% of some regulatory requirement before a deadline, that reduces the
            risk of fines, etc. enormously. The final few low-value, and presumably
            low-risk, items will likely be not used much anyway. Also, if the
            regulators come to inspect, they are likely to be more lenient if you
            have delivered 90% of what is required at the deadline rather than
            having 100% “dev-complete” or “in test”.</p>
        </div>
      </div>
    </div>
...
<style>
  ...
  .walkthrough-graph { height: 100%; margin-left: 8px; width: 310px; margin: 0 auto;
    background-repeat: no-repeat; background-position-x: right; background-position-y: center; }
  .conclusions p { text-align: left; }
  .risk-graph { background-image:  url("../../assets/img/risk.png"); }
  .value-graph { margin-left: 8px; background-image:  url("../../assets/img/value.png"); }
</style>
